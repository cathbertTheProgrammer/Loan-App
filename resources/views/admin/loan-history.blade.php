                                ...
                                <tr>
                                    ...
                                    @if ($loan->status == 'ACTIVE')
                                        <td>
                                            <div
                                                ...
                                                <i class="bi bi-check-circle"></i>{{ $loan->status }}</div>
                                        </td>
                                    @elseif (in_array($loan->status, ['PENDING', 'VERIFIED', 'RECOMMENDED', 'APPROVED']))
                                        <td>
                                            <div
                                                class="badge rounded-pill text-info bg-light-info p-2 text-uppercase px-3">
                                                <i class="bi bi-arrow-clockwise"></i>{{ $loan->status }}</div>
                                        </td>
                                    @elseif (in_array($loan->status, ['DISABLED', 'DELETED']))
                                        <td>
                                            <div
                                                class="badge rounded-pill text-danger bg-light-danger p-2 text-uppercase px-3">
                                                <i class="bi bi-x-circle"></i>{{ $loan->status }}</div>
                                        </td>
                                    @endif
                                    ...
                                    <td>
                                        <div class="d-flex order-actions">
                                            <a href="{{ route('admin.specificLoan', ['loan' => $loan->id]) }}">
